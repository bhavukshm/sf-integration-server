...
- `faq.js` — inbound FAQ API: `GET /api/faqs` guarded by a single static `X-API-KEY` header (no Basic/JWT/OAuth), compared with `timingSafeEqual` against `FAQ_API_KEY`. Simplest possible auth guard in the repo — a good reference when the scenario doesn't call for more.
- `mcp.js` — inbound MCP (Model Context Protocol) server: `POST /api/mcp` exposes a `search_case_resolutions` tool (mock case-resolution knowledge base) over the Streamable HTTP transport, in stateless mode (fresh `McpServer`/transport per request). Auth is checked *inside the tool handler*, not as route middleware — `initialize`/`tools/list` are unauthenticated (there's nothing at the protocol layer to gate), and the handler reads the raw `X-MCP-API-KEY` header via `extra.requestInfo.headers` and throws if it's missing/wrong (the SDK turns a thrown error into a normal `CallToolResult` with `isError: true`, not an HTTP 401). Declares both `inputSchema` and `outputSchema` (zod) — the SDK publishes both in `tools/list` and validates the handler's `structuredContent` against `outputSchema` at runtime. Deliberately mocked (no outbound call) so pagination (`startingPage`/`endingPage`/`singlePageRecordCount`, mirroring a real 3rd-party KB/RAG server's contract) and image delivery (`imageUrl` field + optional inline base64 image content blocks, served by the public `GET /api/kb/images/:id` route) can be exercised before wiring up the real integration.
